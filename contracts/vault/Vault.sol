...
import "../interfaces/IWithdrawVault.sol";
import "../interfaces/IVault.sol";
import "../libraries/SingleAdminAccessControl.sol";
import "../interfaces/IStakedToken.sol";

contract Vault is IVault, Pausable, SingleAdminAccessControl, ReentrancyGuard {
  ...
  //StakedTokens list
  mapping(address => IStakedToken) public supportedTokenToStakedToken;
  mapping(address => address) public stakedTokenToSupportedToken;
  ...
  constructor(
    ...
    for (uint256 i = 0; i < len; i++) {
      ...
      IStakedToken stakedTokenTemp = IStakedToken(_stakedTokens[i]);
      supportedTokenToStakedToken[token] = stakedTokenTemp;
      stakedTokenToSupportedToken[address(stakedTokenTemp)] = token;
  ...
  function cancelClaim(uint256 _queueId, address _token) external whenNotPaused OnlyCancelEnable nonReentrant {
    ...
    supportedTokenToStakedToken[_token].mint(msg.sender, amountToMint);
  ...
  function flashWithdrawWithPenalty(
    ...
    uint256 principalPayout = (principalAmount * (BASE - penaltyRate)) / BASE;
    uint256 rewardPayout = (rewardAmount * (BASE - penaltyRate)) / BASE;
    uint256 totalPayout = principalPayout + rewardPayout;
    uint256 fee = totalAmount - totalPayout;

    require(getContractBalance(_token) >= totalPayout, "not enough balance");

    tvl[_token] -= principalAmount;

    if (principalPayout > 0) {
      IERC20(_token).safeTransfer(msg.sender, principalPayout);
    }
  ...
  function _getExchangeRate(address _token) internal view returns (uint256 exchangeRate) {
    uint256 totalSupplyStakedToken = supportedTokenToStakedToken[_token].totalSupply();
    if (totalSupplyStakedToken == 0) {
      exchangeRate = 1e18;
    } else {
      exchangeRate =
  ...
  function transferOrTransferFrom(
    ...
    uint256 tokenBefore = getStakedBalance(from, token);
    require(tokenBefore >= amount, "balance");
    if (msg.sender != from) {
      require(supportedTokenToStakedToken[token].allowance(from, msg.sender) >= amount, "allowance");
      supportedTokenToStakedToken[token].updateAllowance(from, msg.sender, amount);
      supportedTokenToStakedToken[token].safeTransferFrom(from, to, amount);
    } else {
      supportedTokenToStakedToken[token].safeTransferFrom(msg.sender, to, amount);
    }
  ...
  function distributeStaked(
    ...
    uint256 assetAmount = (amount * exchangeRate) / 1e18;

    assetsInfo.stakedAmount += assetAmount;

    totalStakeAmountByToken[token] += assetAmount;
    tvl[token] += assetAmount;

    emit StakedDistributed(token, to, amount, enableHistoricalRewards);
  }
  ...
  function addSupportedToken(
    ...
    setRewardRate(_token, _newRewardRate);

    IStakedToken stakedToken = IStakedToken(_stakedToken);
    supportedTokenToStakedToken[_token] = stakedToken;
    stakedTokenToSupportedToken[address(stakedToken)] = _token;
  ...
  function getStakedBalance(address _user, address _token) public view onlySupportedToken(_token) returns (uint256) {
    return supportedTokenToStakedToken[_token].balanceOf(_user);
  }
